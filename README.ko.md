...
- Codex permission-profile 시스템 기반 생성 profile
  ...
  기본 설치됩니다. 전용 profile은 `approval_policy = "on-request"`와
  `approvals_reviewer = "auto_review"`를 설정하고, 정확한 절대 launcher rule은
  `prompt`를 사용해 provider 호출을 그 reviewer로 보냅니다. Bootstrap은 owner의
  기본 `config.toml`에 있는 approval key를 교체하지 않습니다.
...
세 단계면 기본 approval 설정은 그대로 두면서 전용 triad profile에서 automatic
review를 사용하는 설치를 마칠 수 있습니다. 이 섹션 아래는 모두 선택입니다.
...
3. **사용자가 실행하는 runtime setup.** 플러그인 installer는 임의의
   ...
   provider 로그인이나 model probe를 실행하지 않습니다. 설치된 절대 launcher rule은
   wrapper command에 `prompt`를 적용하고, 전용 profile은 대상 prompt를 Codex automatic
   review로 보냅니다. Owner가 해당 triad skill을 명시적으로 호출했고 승인된 provider
   input에서 credential, token, cookie, authentication file을 제외했다면, 그 요청에는
   reviewer가 필요로 하는 authorization context가 담깁니다. Denial, timeout, authorization
   누락, unsafe input은 계속 fail closed입니다.
...
기본적으로 정확한 launcher rule은 `prompt`를 사용합니다. *전용 triad session 전체에서
interactive 승인 요청을 없애고 hardened wrapper 환경을 고정해 시작하려는 경우에만*
이 설정을 사용하세요. `TRIAD_CODEX_PROFILE_APPROVAL_POLICY=never`와 managed shell
entry를 bootstrap 명령에 추가합니다.

이 명시적 설정은 전용 profile에 `approval_policy = "never"`를 생성하고 세 managed
launcher rule을 `allow`로 다시 써서 advanced no-prompt 예외를 유지합니다. Profile에는
`approvals_reviewer = "auto_review"`가 계속 들어 있지만 approval policy가 `never`이면
automatic review는 비활성입니다.

> **경고 — session 전체 적용.**
> `approval_policy=never`는 이 plugin만이 아니라 triad Codex session 전체에
> 적용됩니다. 생성된 launcher rule은 user-layer rule이므로 그 `allow` 결정은
> profile-local이 아닙니다. 그 session에서는 관련 없는 작업을 하지 말고, automatic-review
> 자세로 돌아가기 전에 기본 bootstrap 명령을 다시 실행하세요.
...
environment dump, 관련 없는 path를 제외해야 합니다.

기존 Git worktree가 일반 및 formal review의 유일한 source root입니다. Leader는
uncommitted changes, revision range, 단일 commit 중 scope를 선택하고, 고정된 read-only
Git argv로 trusted Git status/diff를 한 번 캡처한 뒤 모든 reviewer에게 동일한 worktree
root, scope, objective, 캡처 결과를 제공합니다. Diff는 시작점이지 review boundary가
아닙니다. 각 no-edit reviewer는 관련된 변경되지 않은 caller, consumer, test, build
file, configuration, governing documentation을 필요에 따라 직접 읽고 검색합니다.
Reviewer는 candidate code, test, build, hook, generated script를 실행하지 않습니다.

Leader는 review 전에 `HEAD`, 선택한 diff, 전체 untracked-path inventory, untracked
content의 Git object hash로 가벼운 fingerprint를 기록하고 모든 leg이 끝난 뒤 다시
계산합니다. Fingerprint가 달라지면 그 round는 invalid이고, 같을 때만 reconcile할 수
있습니다. Reviewer가 실행 중일 때 leader는 worktree를 편집하지 않습니다.

기본 경로는 source copy, packet, manifest, generated related-file list, snapshot을
만들지 않습니다. Source archive는 formal gate의 숨은 prerequisite가 아닙니다. 작은
review record에는 review ID, scope, pre/post fingerprint, 정확한 provider command,
reviewer output만 남길 수 있으며 copied source나 authentication material은 넣지
않습니다. Legacy packet-bound wrapper option은 명시적 compatibility 용도로 남을 수
있지만 설치된 skill과 default gate는 사용하지 않습니다. Commit, push,
install/update, merge, release, publication은 계속 각각 별도의 owner authorization이
필요합니다.
...
않으므로 경계는 선택한 `--cwd` worktree, 요청한 wrapper sandbox, Codex
profile/rules에 의존합니다.

## 문제 해결 (Troubleshooting)

| 증상 | 원인 | 해결 |
|---|---|---|
| Wrapper 호출이 automatic review 대신 수동 승인을 기다림 | 전용 profile이 활성화되지 않았거나, 더 높은 precedence의 project/managed 설정이 다른 reviewer를 선택했거나, 생성 rule이 없거나 stale함 | Profile과 rules를 켠 상태로 새로 출력된 bootstrap 명령을 다시 실행한 뒤 새 `codex --profile triad-codex-dispatch --search` session을 시작하세요. 계속되면 더 높은 precedence policy를 확인하세요. |
| Automatic review가 wrapper 호출을 거부함 | Authorization이 없거나 범위를 벗어났거나, provider-visible input에 제외 대상 credential material이 있거나, reviewer가 다른 unsafe 조건을 찾음 | Denial을 우회하지 마세요. Input 범위를 줄이거나 필요한 owner authorization을 받은 뒤 다시 시도하고, 그럴 수 없으면 중단하세요. |
...
| `66` | 구조화 출력(`--pydantic`) 스키마 검증 실패 | `schema-fail is terminal for that invocation`; leader가 판단한 뒤 explicit new invocation을 만들 수 있습니다. Worktree-first formal 경로는 legacy packet-bound schema를 요구하지 않습니다. |
...
- **wrapper containment은 프로세스/권한 수준이지 OS 수준 confinement이 아닙니다.**
  ...
  Codex profile/rules, 커밋 전 사용자 검토에 의존합니다.
...
- 생성된 command rules는 `~/.local/bin` 또는 `TRIAD_BOOTSTRAP_BIN_DIR` 아래의 정확한
  ...
  호출합니다. Rules는 기본적으로 `prompt`를 사용하고 명시적으로 bootstrap한
  `approval_policy=never` 자세에서만 `allow`를 사용합니다.
