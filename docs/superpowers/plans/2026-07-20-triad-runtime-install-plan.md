...
- Produces: installer-owned runtime context for the absolute Codex executable, sibling launcher directory, absolute profile path, absolute rules path, explicit rules-enabled posture, install-time Gemini vendor availability, canonical source-bin root, exact wrapper paths and digests, pinned Python interpreter, trusted PATH, per-provider vendor pins, and fixed closure-member digests; caller environment and mutable `PATH` cannot override it.
...
- Produces internal CLIs: `install-preflight --manifest M --codex-home H --launcher-root L --codex-bin C --profile-name P --rules-name R` and `publish-install-group --manifest M --codex-home H --launcher-root L --codex-bin C --profile-name P --rules-name R`; bootstrap derives and freezes these literals independently before it creates the manifest and passes the same values to both calls. The fixed schema names profile, rules, opt-out, setup, and doctor targets/candidates, enabled posture, the equality-check copy of the absolute Codex executable, and equality-check launcher directory, while roots, exact executable authority, validated variable basenames, and marker policies are not self-authorized by the manifest.
...
Profile, rules/opt-out posture, `triad-setup`, and `triad-doctor` form one rollback-capable publication group. Bootstrap preflights every live target and renders every enabled candidate without changing a live entry. `publish-install-group` performs the authoritative candidate validation and native-verifies the rules candidate inside the same process before its first mutation, using the separately supplied exact Codex executable and launcher root after equality-checking the manifest copies. It then revalidates candidate identity/content and snapshots exact prior existence, bytes, and modes immediately before publication. After the first publication, any candidate replace, opt-out removal, chmod, or directory-fsync failure restores every rollback-owned original entry (including rollback-owned absence) and fsyncs the affected directories before returning failure. Both runtime commands publish from prepared candidates, never a sequential render-after-mutation loop. Deterministic failure injection covers disable-state creation, opt-out removal, each artifact replacement, chmod/preparation, native verification, and directory fsync; every failed run must preserve the complete prior artifact set byte-for-byte and posture-for-posture except any concurrently substituted live destination explicitly classified `state-preserved`.

The transaction helper owns target validation and rollback. `_read_regular_nofollow` uses `lstat`, `O_RDONLY | O_NOFOLLOW | O_NONBLOCK`, `fstat`, `samestat`, and a bounded read; it never calls a following `Path.read_text()` or `grep` on a live target. Every candidate is a fsynced regular file created in its target directory with final mode already applied. Each CLI invocation loads and validates the manifest exactly once into one immutable in-memory object; no preflight, verification, snapshot, publication, or fsync phase reopens the manifest path. Both internal CLIs require the full separately supplied installer-derived authority shown in the interface; manifest fields may only equality-match it and may not self-authorize roots or names. `P` is a validated logical profile stem whose exact target is `H / f"{P}.config.toml"`; `R` is a validated rules basename whose exact target is `H / "rules" / R`. Every raw path rejects lexical `..`, checks every existing component for symlinks before resolution, and is then proved canonically confined: the manifest is a direct child named `.triad-install-manifest.<random>` under the launcher root; `launcher_dir` equals that root; setup/doctor targets have the code-owned basenames `triad-setup` and `triad-doctor`; profile/rules targets equal those exact `P`/`R` formulas; opt-out equals exact `str(rules_target) + ".opt-out"`; candidates have the same canonical parent as distinct targets; canonical aliases fail uniqueness. `publish-install-group` reads descriptor-bound candidate bytes once, creates transaction-private sealed copies in mode-0700 staging directories under each target parent, and never publishes the externally named candidate. Native rules verification consumes the sealed rules copy. Immediately after each portable `os.replace`, the transaction opens the destination with `O_NOFOLLOW`, compares identity, mode, size, and SHA-256 with the still-held sealed artifact, and rolls back transaction-owned entries before commit on any mismatch. A stably observed foreign live destination always takes precedence as `state-preserved`, including when the private source was consumed before Python returned from `os.replace`; rollback never overwrites that inode, and the operation fails loudly with the affected path. For live publication/removal mutation state, only uninspectable evidence with no proven foreign live destination remains conservatively `rollback-owned`; unknown-pending private preparation follows the separate cleanup rule below. This resolves the otherwise observationally indistinguishable portable cases of source substitution and a legitimate replace followed by concurrent destination substitution in favor of preventing data loss. This private-copy plus destination-verification protocol is the strongest shared Python/POSIX contract available on both Ubuntu 24.04 and macOS; it does not rely on `chflags`, `renameat2`, `/proc`, `fexecve`, or `/dev/fd`. Any manifest, target, or candidate substitution between preflight, verification, and publication either becomes irrelevant to the sealed bytes or fails the destination check. Publication order is profile; enabled rules then exact managed opt-out removal, or disabled exact opt-out then managed-rules removal while an unmanaged regular rules file is preserved; setup; doctor last; cleanup of private staging; then stable de-duplicated parent-directory fsync derived from all five entries, including preserved/absent actions. Each entry has explicit `pending`, `rollback-owned`, or `state-preserved` state: register `pending` before a mutating syscall, clear it if no mutation occurred or local restoration completed, promote it to `rollback-owned` after a live mutation, and never snapshot-restore a concurrent target classified `state-preserved`; all earlier `rollback-owned` entries still revert. On abnormal unwinding, every remaining `pending` entry is conservatively treated as `rollback-owned` unless that operation proves no mutation/local restoration or explicitly classifies the target `state-preserved`; a proven foreign live leaf is always the latter, regardless of source presence. Tests perform the real replace/quarantine/unlink and raise before normal promotion. Sealing, quarantine, publication, rollback, and rollback loops handle `BaseException`: after successful restoration the original `KeyboardInterrupt` or `SystemExit` is re-raised with its traceback, while a secondary restore failure becomes fatal `rollback incomplete` evidence and every remaining restore is still attempted. Rollback walks rollback-owned targets in reverse, recreates prior bytes and mode through fresh same-directory candidates, destination-attests each restore, restores absence where applicable, and distinguishes a normal reverted failure from fatal `rollback incomplete` with affected paths. The direct callable `fail_at` seam covers manifest-path substitution, sealed-copy substitution, every post-destination check, every replace/unlink, and every parent-fsync boundary and compares the complete rollback-owned state after each injected failure while proving state-preserved targets remain unmodified.

Registration of a random private pathname is recoverability metadata, not proof that the transaction owns the inode currently found there. Staging and quarantine names are selected and registered before `mkdir` or `os.open(O_CREAT | O_EXCL | O_NOFOLLOW)`. Destructive cleanup is authorized only after the creating call returned a descriptor or the transaction captured and retained an exact no-follow device/inode identity. If a create-like syscall performs its native mutation but Python raises `KeyboardInterrupt` or `SystemExit` before returning that evidence, the pathname remains `unknown-pending`: cleanup must not traverse, unlink, or remove whatever is now at that name. It instead records `state-preserved`/`cleanup incomplete` with the exact path, continues every other provably owned cleanup, and chains the original control-flow exception. This is an explicit portable residual because neither POSIX `mkdir` nor Python's create wrappers atomically return ownership evidence on both Ubuntu 24.04 and macOS. Once exact descriptor-bound identity is available, later `fstat`, close, chmod, write, fsync, or cleanup failures use no-follow, directory-FD-relative identity checks, attempt every cleanup, and re-raise the identical control-flow exception after complete recovery.

Portable same-UID cleanup has one narrower accepted residual. Retained parent/leaf descriptors, `dir_fd`, `O_NOFOLLOW`, and device/inode attestation prevent ancestor traversal and detect substitutions observed before or after cleanup, but Python/POSIX `unlink` and `rmdir` are pathname operations and cannot condition deletion on the attested inode. A noncooperating same-UID process with namespace-write access can rebind an attested private staging or quarantine name in the final check-to-delete window. Every observed mismatch or uninspectable state is preserved loudly and all other cleanup continues; an exactly matched private entry is removed only as best-effort cleanup. The workflow does not claim zero foreign deletion under an actively racing same-UID namespace adversary. This plan selects best-effort cleanup because that actor is outside the product threat model and already has authority to remove these private entries directly; it does not weaken the no-clobber rule for live publication/removal destinations. A strict same-UID-adversary posture would require preservation-only cleanup and path-specific persistent residue, which this plan does not enable. Random names and mode 0700 reduce exposure but are not ownership proof.

Managed removal never checks and then directly unlinks a live name. It preselects and registers a random same-directory quarantine name, creates its placeholder with `os.open(O_CREAT | O_EXCL | O_NOFOLLOW)`, captures descriptor-bound identity before any later fallible preparation, and atomically renames the entry onto that exact placeholder. It opens and verifies the quarantined inode/content through `O_NOFOLLOW`, then attempts best-effort unlink after descriptor/inode-observing an exact managed match, subject to the private-cleanup residual above. A mismatch is restored without clobbering a newly occupied target; if safe restoration is impossible, both entries are preserved and the command fails loudly. If the rename consumed the source, a foreign live target and the exact original quarantine are both `state-preserved`, even when interruption occurred before `os.replace` returned. The rules opt-out marker is exact: marker plus any leading or trailing byte is unmanaged and preserved.

Step 1 adds named direct tests for `live_probe_environment`, `emit_live_report`, marker-bearing and looping pinned symlinks, pinned FIFO/non-regular leaves, regular markerless launcher replacement between static and live phases, every group target as live/dangling symlink and FIFO, managed hardlink peers, profile/rules/opt-out quarantine removal, enabled and disabled transaction actions, and each parent-directory fsync. Every pinned non-regular, markerless, or loop case is one stable JSON finding with zero probes. Path tests cover ancestor symlinks, lexical `..`, canonical aliases, root escape, exact topology, and disagreement between the manifest and separately supplied CLI authority. A between-CLI test substitutes each live target and external candidate after preflight; an after-verification test substitutes the external candidate and proves the sealed verified bytes are published instead; an `os.replace` interception swaps the private source and requires post-destination mismatch handling without overwriting a proven foreign inode. Sealed wrapper tests use a real bootstrap-generated launcher, replace both its original repo wrapper and one imported dependency after sealing, and prove only the sealed execution closure runs. The publication matrix injects normal exceptions plus `KeyboardInterrupt` and `SystemExit` at sealing, replace-call, post-destination, post-quarantine, post-unlink, parent-fsync, and rollback double-fault boundaries; it compares the complete five-entry existence/bytes/mode snapshot, requires original control-flow exception propagation after successful recovery, and requires affected-path `rollback incomplete` evidence after a secondary failure. It includes real-replace-then-foreign-substitution-before-return for both publication and quarantine, proves the foreign live target survives, proves an exact original quarantine also survives, and still reverts all earlier rollback-owned entries. Removal tests substitute the target before quarantine and require mismatches to be restored/preserved. Preparation tests cover native create followed by a pre-return `KeyboardInterrupt`/`SystemExit`, post-return `fstat`/close/write/fchmod/fsync failures, and unknown-path substitution. They require zero live-target mutation; exact cleanup and original exception identity once ownership evidence exists; and loud path-specific `state-preserved`/`cleanup incomplete` without deleting an unknown pending leaf or directory when ownership evidence was never returned. Two no-skip macOS/Ubuntu characterization tests document, rather than falsely close, the final same-UID residual: one retains parent and owned FDs, substitutes a foreign hardlinked leaf immediately before the real `unlink(..., dir_fd=...)`, and proves the current foreign binding is removed while its peer preserves inode/link-count evidence; the other substitutes a foreign empty directory immediately before real `rmdir(..., dir_fd=...)` and proves the current binding is removed. They are labeled portability/residual characterization and remain separate from production tests that require every substitution visible before the destructive syscall to be preserved loudly.
...
Every non-Codex live probe includes `--cwd <canonical-workspace>`. Bootstrap embeds an installer-owned immutable runtime context containing the exact canonical source-bin root, exact wrapper paths, pinned interpreter, trusted PATH, per-provider vendor pins including explicit absence, launcher/closure SHA-256 values, and install-time Gemini posture; the generated doctor overwrites caller environment with these literals. Before the first probe, doctor resolves only code-owned allowlisted relative closure members beneath that source root, descriptor-reads and digest-checks them, compiles Python bytes without executing them, and creates one runnable private mode-0700 bundle. Its temporary base is selected independently of caller `TMPDIR` and the resulting tree must resolve outside the canonical workspace, source-bin root, and persistent log/debug roots or fail before any probe. The closure is explicit: each provider's real wrapper and `_common.py`; Antigravity also `_agy_settings.py` and `_pty.py`; Gemini also `policies/gemini-readonly.toml`. A code-owned sealed-entrypoint template applies the same scrub list, trusted PATH, `TRIAD_REQUIRE_PINNED_VENDOR`, and provider pin from runtime context, invokes the pinned interpreter with `-E -S -B`, and references only sealed-tree wrapper/import/resource paths; runtime does not parse a marker-bearing launcher to authorize these values and no original repository path remains in the executed closure. All non-Codex probes execute those sealed entrypoints. Live-doctor wrapper audit/run logs are intentionally ephemeral inside the sealed tree and are removed with it; the final doctor report is the persistent diagnostic, while ordinary public wrapper invocations retain their existing persistent logs. Any marker, digest, dependency, resource, bundle, or attestation failure returns one stable failed report with `probes: []`, and later replacement of an original launcher, wrapper, dependency, or policy cannot affect the sealed execution bytes. The Codex `login status` leg is a local non-model/non-source status check against the installer-pinned package entrypoint; because npm shims depend on their relative package layout, it is re-attested immediately before spawn rather than naively copied. The probe environment hard-sets `TRIAD_WRAPPER_ALLOWED_ROOTS` to that same workspace, `TRIAD_WRAPPER_HARDENED=1`, and `TRIAD_CLAUDE_ENFORCE_SANDBOX=1`; Google additionally receives `AGY_SETTINGS_LOCK_TIMEOUT=30` and `AGY_NO_HEADLESS_AUTOAPPROVE=1`. This prevents caller-directory disclosure, caller-expanded lock time, and the headless soft-deny second vendor run. Any static integrity finding stops all wrapper/provider execution and returns a failed live report with an empty probe list.
...
Then pull/resolve `ubuntu:24.04` once, record its immutable repo digest plus `python3 --version`, `python3 -m pytest --version`, and `bash --version`, and run that digest-pinned image with `--init` and the same worktree mounted read-only at `/repo`. Install test-only packages only inside the ephemeral container, set `DEBIAN_FRONTEND=noninteractive`, set `PYTHONPYCACHEPREFIX=/tmp/triad-pyc` so pytest imports and `py_compile` write no bytecode into the source mount, and disable pytest's repository cache with `-p no:cacheprovider`. The recorded command is this argv shape with `<resolved-ubuntu-24.04-digest>` and `<absolute-worktree>` replaced by their recorded literal values: `docker run --rm --init --mount type=bind,src=<absolute-worktree>,dst=/repo,readonly --workdir /repo --env DEBIAN_FRONTEND=noninteractive --env PYTHONPYCACHEPREFIX=/tmp/triad-pyc ubuntu@<resolved-ubuntu-24.04-digest> bash -lc 'apt-get update -qq && apt-get install -y -qq --no-install-recommends python3 python3-pytest && python3 --version && python3 -m pytest --version && bash --version | head -n 1 && python3 -m pytest tests/test_bootstrap.py tests/test_triad_runtime.py -q -rs -p no:cacheprovider && python3 -m py_compile bin/triad_runtime.py && bash -n scripts/bootstrap.sh'`. No platform skip may cover a core transaction, attestation, removal, live-probe, or bootstrap contract. The sole expected Ubuntu skip is the named case-insensitive-filesystem bypass regression: on case-sensitive ext4 its mixed-case path is distinct and the bypass cannot exist; record that exact reason. Record the expanded exact command and result before Step 5.
...
Then reuse Task 3's recorded immutable Ubuntu 24.04 digest and read-only `/repo` mount with `--init`, `DEBIAN_FRONTEND=noninteractive`, and `PYTHONPYCACHEPREFIX=/tmp/triad-pyc`. Expand and record the literal digest and worktree path in this command before running it: `docker run --rm --init --mount type=bind,src=<absolute-worktree>,dst=/repo,readonly --workdir /repo --env DEBIAN_FRONTEND=noninteractive --env PYTHONPYCACHEPREFIX=/tmp/triad-pyc ubuntu@<resolved-ubuntu-24.04-digest> bash -lc 'apt-get update -qq && apt-get install -y -qq --no-install-recommends python3 python3-pytest && python3 --version && python3 -m pytest --version && bash --version | head -n 1 && python3 tests/test_gemini_sandbox.py && python3 -m pytest tests/test_bootstrap.py tests/test_triad_runtime.py -q -rs -p no:cacheprovider && python3 -m py_compile bin/triad_runtime.py bin/gemini_wrapper.py && bash -n scripts/bootstrap.sh'`.
