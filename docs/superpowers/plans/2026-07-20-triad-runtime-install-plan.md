...
The transaction helper owns target validation and rollback. `_read_regular_nofollow` uses `lstat`, `O_RDONLY | O_NOFOLLOW | O_NONBLOCK`, `fstat`, `samestat`, and a bounded read; it never calls a following `Path.read_text()` or `grep` on a live target. Every candidate is a fsynced regular file created in its target directory with final mode already applied. `publish-install-group` repeats all live-target validation, records each candidate's device, inode, size, and SHA-256 before native verification, runs verification internally, then rechecks the same identity and digest before capturing the authoritative live snapshot and beginning mutation. Any target or candidate substitution between preflight, verification, and publication fails before publication. Publication order is profile; enabled rules then managed opt-out removal, or disabled opt-out then managed-rules removal while an unmanaged regular rules file is preserved; setup; doctor last; then stable de-duplicated parent-directory fsync. Rollback walks touched targets in reverse, recreates prior bytes and mode through fresh same-directory candidates, restores absence where applicable, attempts every restore, and distinguishes a normal reverted failure from fatal `rollback incomplete` with affected paths. The direct callable `fail_at` seam covers the after-verification substitution boundary plus every replace, unlink, and parent-fsync boundary and compares the entire five-entry snapshot after each injected failure.

Step 1 adds named direct tests for `live_probe_environment`, `emit_live_report`, marker-bearing and looping pinned symlinks, pinned FIFO/non-regular leaves, every group target as live/dangling symlink and FIFO, managed hardlink peers, profile/rules/opt-out removal, enabled and disabled transaction actions, and each parent-directory fsync. Every pinned non-regular or loop case is a stable JSON finding with zero probes. A between-CLI test substitutes each live target and candidate after preflight/native verification and requires rejection before publication. The publication matrix injects one failure at every replace/unlink/fsync boundary and compares the complete five-entry existence/bytes/mode snapshot; a separate double-fault case fails publication and one rollback restore, requiring the exact `rollback incomplete` fatal classification and affected path list. Preparation failures at create/write/fchmod/fsync prove zero live mutations and zero candidate residue.
