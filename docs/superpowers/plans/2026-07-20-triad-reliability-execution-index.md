...
- Registering a random temporary pathname before a mutating syscall does not prove ownership. Cleanup may destructively remove only a descriptor- or device/inode-attested private entry; a create call interrupted before returning ownership evidence leaves a loud path-specific `state-preserved`/`cleanup incomplete` result. A stably observed foreign live destination always takes precedence over ambiguous source state and is never snapshot-restored. For private cleanup only, descriptor/inode checks plus `dir_fd` cannot make the final portable `unlink`/`rmdir` conditional on that inode; best-effort deletion retains an accepted active-same-UID rebinding residual, characterized without skip on both macOS and Ubuntu. Strict protection against that out-of-scope actor requires preservation-only cleanup and persistent residue.
