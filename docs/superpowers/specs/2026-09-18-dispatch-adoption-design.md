...
| S4: Claude provenance receipt | Medium; separate evidence-only design | Preserves session/model/usage/denial metadata beyond the capped stdout head. | Bounded sanitized receipt nested in the existing audit, outside verdict; denials are not automatic invalidation; estimates are not billing. |
