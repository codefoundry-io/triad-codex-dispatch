...
Keep `_run_once`'s existing signature and `RunResult`'s public artifact schemas. Add one private defaulted `RunResult._stdin_delivery_failed: bool = False`. Audit and failure records in `_common.audit` and `_common.emit_run_log` explicitly enumerate fields; they do not serialize the dataclass wholesale. The private flag therefore does not expand those schemas. Human evidence uses the existing `extraction_error` field with a fixed prompt-free diagnostic, and the existing `unknown` classification/`EXIT_CLI_FAIL` failure route. No classifier token, exit code, or `LegVerdict` field is added.
