...
Linux/WSL2 전제: Codex 자체 local sandbox를 안정적으로 쓰려면 `bubblewrap`
(`bwrap`)가 필요하다. Ubuntu 20.04에서는 `bubblewrap`, `git`, `jq`를
설치하거나 제공한다. Ubuntu 20.04 기본 Python은 오래됐으므로 bootstrap 전에
`python3 --version`이 `python3 >= 3.12`인지 확인한다. Ubuntu 20.04는 보통 bash로
시작하므로 아래 shell 설정은 기본으로 `~/.bashrc`에 쓰고, zsh 사용자일 때만
`~/.zshrc`로 전환한다. `~/.local/bin`이 `PATH`에 있어야 하며, 그렇지 않으면
`TRIAD_BOOTSTRAP_BIN_DIR`를 이미 `PATH`에 있는 launcher directory로 지정한다.
