...
설치 후 새 Codex thread를 시작해야 plugin cache의 skill이 로드된다. 배포 skill은
`skills/` 아래에만 둔다. 이 checkout에 repo-local `.agents/skills/` mirror를
남기면, plugin을 설치한 상태로 이 repo에서 개발할 때 triad skill이 중복으로 보인다.
