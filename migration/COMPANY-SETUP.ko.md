...
환경에서만 켠다. profile, rules, custom-agent, `skills.config`를 바꾼 뒤에는 기존 Codex session을 재시작해야 한다.
...
`scripts/bootstrap.sh --check`는 toolkit이 관리하는 항목만 확인하거나 설치한다.
